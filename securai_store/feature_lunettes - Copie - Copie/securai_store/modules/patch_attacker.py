...
class PatchAttacker:
    ...
    Workflow live (T1) :
      1. generate_patch(target_emb, target_name)  — calcul PGD unique (~3s)
    ...
    def apply_patch(self, face_bgr: np.ndarray,
        ...
        Args:
            ...
            perturbation: Tenseur (1,3,160,160) issu de generate_patch().

        Retourne l'image BGR avec les lunettes adversariales appliquées.
        """
        if perturbation is None:
            return face_bgr

        orig_h, orig_w = face_bgr.shape[:2]
        img_tensor = self._preprocess(face_bgr)          # (1,3,160,160)
        mask_np    = self.create_glasses_mask(160, 160)   # (160,160)
