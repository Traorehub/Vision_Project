...
class PatchAttacker:
    ...
    def generate_patch(self, target_embedding: torch.Tensor,
                       target_name: str = "__unnamed__",
                       reference_face_bgr: np.ndarray | None = None) -> torch.Tensor:
        """
        Calcule et met en cache la perturbation PGD pour une cible.

        Utilise ``attack()`` sur un visage de référence (webcam ou photo enrôlée).
        Sans référence, fallback image grise neutre (peu efficace en live).
        """
        if reference_face_bgr is None:
            reference_face_bgr = np.full((160, 160, 3), 128, dtype=np.uint8)
            print("[PatchAttacker] Avertissement : pas de visage de référence — patch sur fond neutre.")
        self.attack(reference_face_bgr, target_embedding, target_name=target_name)
