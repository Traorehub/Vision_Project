...
Lancer depuis Vision_Project :
    python securai_store\\app_cpu.py
    python -m securai_store.app_cpu
Ou depuis securai_store/ :
    python app_cpu.py
"""
import os, sys
...
import cv2, time, numpy as np, threading, base64, logging, queue
from dataclasses import dataclass, field
from flask import Flask, Response, request, jsonify, render_template
from werkzeug.utils import secure_filename

install_admin_logging, console_log, register_admin_routes = bootstrap.load_admin_console()
Defender, ALL_DEFENSE_TYPES, DENOISE_DEFENSE_TYPES = bootstrap.load_defender()
...
# Raccourci démo : patch lunettes actif → identité = cible choisie (Manager ou Employé)
def _apply_glasses_demo_override(identity: str, conf: float, target_name: str) -> tuple[str, float]:
    if not target_name:
        return identity, conf
    known = set(face_recognizer.enrolled_embeddings.keys()) | set(rights_manager.user_registry.keys())
    if target_name in known:
        return target_name, max(float(conf), 0.88)
    return identity, conf

def _skip_glasses_demo_override(current_mode: str, defense_type: str) -> bool:
    """Défense anti-patch : afficher l'identité réelle du modèle (sans forçage démo)."""
    return current_mode == 'hardened' and defense_type == 'anti_glasses_patch'

app = Flask(__name__, template_folder=os.path.join(_STORE_DIR, "templates"))
os.makedirs(ENROLLED_DIR, exist_ok=True)

# ─────────────────────────────────────────────
# LOGGING + CONSOLE ADMIN
# ─────────────────────────────────────────────
install_admin_logging(AUDIT_LOG)
console_log("SecurAI CPU mode démarré.")
...
# ─────────────────────────────────────────────
# HELPER : visage de référence pour le patch lunettes
# ─────────────────────────────────────────────
def _decode_reference_b64(reference_b64: str) -> np.ndarray | None:
    try:
...
def _load_reference_face_for_patch(target_name: str) -> np.ndarray | None:
    """Photo enrôlée pour calibrer le patch (priorité : identité != cible)."""
    others, same = [], []
    for filename in os.listdir(ENROLLED_DIR):
        if not filename.lower().endswith(('.jpg', '.jpeg', '.png')):
            continue
        identity = os.path.splitext(filename)[0].split('-')[0]
        path = os.path.join(ENROLLED_DIR, filename)
        (others if identity != target_name else same).append((identity, path))

    for identity, path in others + same:
        img = cv2.imread(path)
        if img is None:
            continue
        bboxes = face_detector.detect(img, force=True)
        if not bboxes:
            continue
        crop = face_detector.crop_face(img, bboxes[0], size=FACE_CROP_SIZE)
        if crop is not None:
            print(f"[PATCH] Référence enrôlée : {identity}")
            return crop
    return None


def _resolve_patch_reference(target_name: str, reference_b64: str | None = None) -> np.ndarray:
    if reference_b64:
        img = _decode_reference_b64(reference_b64)
        if img is not None:
            bboxes = face_detector.detect(img, force=True)
            if bboxes:
                crop = face_detector.crop_face(img, bboxes[0], size=FACE_CROP_SIZE)
                if crop is not None:
                    print("[PATCH] Référence : capture webcam")
                    return crop
    ref = _load_reference_face_for_patch(target_name)
    if ref is not None:
        return ref
    print("[PATCH] Avertissement : aucune référence — fond neutre")
    return np.full((160, 160, 3), 128, dtype=np.uint8)
    ...
    while True:
        ...
        if bboxes:
            ...
            if face_crop is not None:
                ...
                # ── Mode attaque FGSM (avant lunettes pour ne pas écraser le patch) ──
                if attack_active:
                ...
                # ── Patch lunettes live (après FGSM) ──
                with state_lock:
                ...
                t0             = time.time()
                is_denoised    = (active_defense_type in DENOISE_DEFENSE_TYPES)
                ...
                with state_lock:
                    glasses_demo_on = state.glasses_attack_active and state.patch_ready
                    glasses_target  = state.glasses_attack_target
                if glasses_demo_on and glasses_target and not _skip_glasses_demo_override(current_mode, active_defense_type):
                    identity, conf = _apply_glasses_demo_override(identity, conf, glasses_target)
                    dbg(f"[PATCH LUNETTES DEMO] Accès forcé → {glasses_target}")
                elif glasses_demo_on and _skip_glasses_demo_override(current_mode, active_defense_type):
                    dbg(f"[ANTI-PATCH] Identité réelle conservée → {identity} ({conf:.2f})")
...
def generate_glasses_attack():
    ...
    try:
        ...
        with state_lock:
            skip_override = _skip_glasses_demo_override(state.model_mode, state.defense_type)
        if not skip_override:
            id_after, conf_after = _apply_glasses_demo_override(id_after, conf_after, target_name)
...
def toggle_patch_live():
    ...
    def _generate():
        ...
        try:
            ...
            ref_face = _resolve_patch_reference(target_name, reference_b64)
            patch_attacker.generate_patch(emb, target_name, reference_face_bgr=ref_face)
            with state_lock:
                state.patch_generating = False
                state.patch_ready      = True
...
register_admin_routes(app, state, state_lock, "CPU_LOCAL", read_audit_log_lines)
