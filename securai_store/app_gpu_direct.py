...
Lancer depuis Vision_Project :
    python securai_store\\app_gpu_direct.py
    python -m securai_store.app_gpu_direct
...
console_log(f"SecurAI {RUN_MODE} démarré — device={GPU_DEVICE}")
    ...
    while True:
        ...
        if bboxes:
            ...
            if face_crop is not None:
                ...
                # ── FGSM d'abord (ne doit pas écraser le patch lunettes) ──
                if attack_active:
...
register_admin_routes(app, state, state_lock, RUN_MODE, read_audit_log_lines)
