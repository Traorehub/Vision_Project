        ...
        * **frame_skip** – nombre de frames à ignorer pour alléger le CPU.
        ...
        for path_opt in paths_to_try:
            if path_opt.is_file() and path_opt.stat().st_size >= 1_000_000:
                try:
                    # Suppress warnings about ultralytics.utils which is not critical
                    import warnings
                    ...
                    # If it's just an ultralytics.utils issue, try anyway
                    if "ultralytics.utils" in str(e) or "No module named" in str(e):
                        try:
                            import warnings
                            with warnings.catch_warnings():
                                warnings.filterwarnings("ignore")
                                # Force load without validation
                                self.model = YOLO(str(path_opt), verbose=False)
                            loaded = True
                            print(f"[FaceDetector] Modèle chargé avec succès depuis : {path_opt.absolute()}")
                            break
                        except:
