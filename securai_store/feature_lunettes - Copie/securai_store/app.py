...
def _local_inference(face_crop: np.ndarray) -> dict:
    """Fallback local via FaceRecognizer quand HF n'est pas accessible."""
    name, confidence = face_recognizer.predict(face_crop)
    access = rights_manager.get_access_level(name)
    if DEBUG:
        print(f"[LOCAL INFER] {name} | conf={confidence:.2f}")
    return {
        'name': name,
...
def _hf_is_available() -> bool:
    if hf_status['available']:
        return True
    return time.time() - hf_status['last_failure'] >= hf_status['retry_after']


def _mark_hf_success() -> None:
    hf_status['available'] = True
    hf_status['consecutive_failures'] = 0
    with state_lock:
        state.hf_available = True


def _mark_hf_failure(reason: str) -> None:
    hf_status['consecutive_failures'] += 1
    hf_status['last_failure'] = time.time()
    if hf_status['consecutive_failures'] >= hf_status['max_failures']:
        hf_status['available'] = False
        with state_lock:
            state.hf_available = False
        logging.warning(f"HF unavailable after {hf_status['consecutive_failures']} failures: {reason}")
    else:
        logging.warning(f"HF transient failure ({hf_status['consecutive_failures']}): {reason}")


def _overlay_patch(frame: np.ndarray, bbox: tuple[int, int, int, int], patched_face: np.ndarray) -> np.ndarray:
    """Replace the detected face region with the patched face crop."""
    x1, y1, x2, y2 = bbox
    h, w = frame.shape[:2]
    ...
    frame[y1:y2, x1:x2] = patched_resized
    return frame
...
def send_to_hf_infer(face_crop: np.ndarray) -> dict | None:
    ...
    Retourne dict {name, confidence, access} ou fallback local si HF est indisponible.
    """
    if not _hf_is_available():
        if DEBUG:
            print("[HF INFER] HF indisponible, fallback local")
        return _local_inference(face_crop)
    ...
    try:
        ...
        if status >= 500 or status in (429, 408):
            _mark_hf_failure(reason)
        else:
            logging.warning(f"HF /infer client error: {reason}")
        return _local_inference(face_crop)
    except requests.exceptions.RequestException as e:
        _mark_hf_failure(str(e))
        return _local_inference(face_crop)
    except Exception as e:
...
def send_to_hf_fgsm(face_crop: np.ndarray, target: str = "Manager_Demo") -> np.ndarray | None:
    ...
    if not _hf_is_available():
        if DEBUG:
            print("[HF FGSM] HF indisponible, FGSM ignoré")
        logging.warning("HF FGSM ignored while HF unavailable")
        return None
    ...
    try:
        ...
        status = e.response.status_code if e.response is not None else 0
        _mark_hf_failure(f"HF FGSM HTTP {status}")
        return None
    except requests.exceptions.RequestException as e:
        _mark_hf_failure(str(e))
        return None
    ...
    while True:
        ...
        if bboxes:
            ...
            if face_crop is not None:
                # ── Patch lunettes live (T1) ─────────────────────────────────
                # Utilise apply_patch() (<5ms) si le patch est pré-calculé,
                # contrairement à attack() (~2s) qui recalcule tout à chaque frame.
                if glasses_attack_active and glasses_attack_target:
                    cached_perturbation = patch_attacker.get_cached_patch(glasses_attack_target)
...
def get_status():
    with state_lock:
        retry_delta = max(0.0, hf_status['retry_after'] - (time.time() - hf_status['last_failure']))
        return jsonify({
            'identity':              state.identity,
            ...
            'hf_available':          state.hf_available,
            'hf_consecutive_failures': hf_status['consecutive_failures'],
            'hf_retry_after':        hf_status['retry_after'],
            'hf_next_retry_seconds': round(retry_delta, 1),
...
def hf_health():
    with state_lock:
        next_retry = max(0.0, hf_status['retry_after'] - (time.time() - hf_status['last_failure']))
        return jsonify({
            'available':             hf_status['available'],
            'consecutive_failures':  hf_status['consecutive_failures'],
            'max_failures':          hf_status['max_failures'],
            'retry_after':           hf_status['retry_after'],
            'next_retry_seconds':    round(next_retry, 1),
            'last_failure_unix':     hf_status['last_failure'],
            'status':                'OK' if hf_status['available'] else 'DEGRADED',
            'note':                  'HF inference is currently being proxied to local fallback when unavailable.',
        })

# ── /api/enrolled_list — liste des identités enrôlées ───────────────────────
...
# ── /api/patch_status — état du patch live (T1) ──────────────────────────────
...
# ── /api/toggle_patch_live — active / désactive l'attaque lunettes live ──────
...
def toggle_patch_live():
    ...
    Corps JSON attendu :
      { "active": true,  "target": "Manager_Demo" }  → génère + active
...
def generate_glasses_attack():
    ...
    if request.form.get('live') == 'true':
        ...
        logging.info(f"Live glasses attack enabled for {target_name}")
        return jsonify({
            "success": True,
            "message": f"Live glasses attack activée pour {target_name}.",
            "target":  target_name,
            "enrolled": enrolled
        })

    if request.form.get('stop') == 'true':
        with state_lock:
            state.glasses_attack_active = False
            state.glasses_attack_target = ''

        logging.info("Live glasses attack désactivée")
        return jsonify({
            "success": True,
            "message": "Live glasses attack désactivée.",
            "enrolled": enrolled
        })
